...
1. 修饰变量，说明该变量不可以被改变；
2. 修饰指针，分为指针常量（pointer const：指向不能变，值可以变）、自身是常量的指针（常量指针，const pointer: 指向可以变，值不能变）和既修饰指针又修饰常量（const pointer const）
...
4. 修饰成员函数，修饰成员函数使得不需要生成对象就可以访问该函数，但是在 __static 函数内不能访问非静态成员__。
